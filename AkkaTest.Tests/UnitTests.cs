...
namespace AkkaTest.Tests
{
    public class UnitTests: TestKit
    {
        ...
        [Fact]
        public void DeviceManager_actor_must_be_able_to_register_a_group_actor()
        {
            var probe = CreateTestProbe();
            var groupActor = Sys.ActorOf(DeviceManager.Props("groupManager"));

            groupActor.Tell(new RequestTrackDevice("group", "device1"), probe.Ref);
            probe.ExpectMsg<DeviceRegistered>(s => s.GroupId == "group");

            groupActor.Tell(new RequestTrackDevice("group2", "device2"), probe.Ref);
            probe.ExpectMsg<DeviceRegistered>(s => s.GroupId == "group2");
        }
    }
}